...
public class Board {
   ...
   // }
     ...
     * 
     */
    public boolean findLegalMove(Move move, int player){ // !! TESTING boolean / int issue
    	...
    	if(cells[i][j].isEmpty()==false){ // checks if cell is empty
    		...
    		for(int k=0; k<directions.length;k++){ // get all the directions one by one
    			...
    			if((j+jDir)>-1 && (j+jDir)<NUM && (i+iDir)<NUM && (i+iDir) >-1){ // checks for an opponent in all the directions
    				if(cells[i+iDir][j+jDir].getPlayer() == opponent){ // !! TESTING cells are Cell type and opponent is int type
    					while((j+(jump*jDir))>-1 && (j+(jump*jDir))<8 && (i+(jump*iDir))<8 && (i+(jump*iDir))>-1){ // search inside the board
    						if(cells[i+jump*iDir][j+jump*jDir].getPlayer() == 0){ // !! TESTING cells are Cell type and 0 is int type
    ...
    public void replaceChip(Move move, Boolean player){ // TESTING boolean / int issue
        ...
        for(int movement=0; movement<directions.length; movement++){
			...
			if((j+jDir)>-1 && (j+jDir)<NUM && (movement+iDir)<NUM && (movement+iDir) >-1){ // checks for an opponent in all the directions
				if(cells[movement+iDir][j+jDir].getPlayer() == opponent){
					possible = true;
				}
			}
			...
			if(possible == true){
				int jump = 2;
				while((j+(jump*jDir))>-1 && (j+(jump*jDir))<8 && (movement+(jump*iDir))<8 && (movement+(jump*iDir))>-1){ // search inside the board
					if(cells[movement+jump*iDir][j+jump*jDir].getPlayer() == 0){ // !! TESTING cells are Cell type and 0 is int type
						break; // if cell empty, pass
					}
					if(cells[movement+jump*iDir][j+jump*jDir].getPlayer() == playing){
						for(int k=1; k<jump; k++){
							cells[movement+k*iDir][j+k*jDir].changeChip();// = playing; // everything between i and j is ours
						}
						break;
					}
				jump++;
